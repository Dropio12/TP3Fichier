...
class Arene:
    ...
    def dans_arene(self, emplacement):
        ...
        """
        if 0 <= emplacement[0] < self.dimension and 0 <= emplacement[1] < self.dimension:
            return True
        return False
    ...
    def rangement(self, joueur_en_cours):
        ...
        """
        self.retirer_les_x()
        valeurs = self.compter_valeurs()
        self.retirer_correspondances(valeurs, joueur_en_cours)
        if self.correspondance_existe(valeurs):
            return True
    ...
    def correspondance_existe(self, comptes):
        ...
        """
        if comptes[2] > 1 or comptes[3] > 1 or comptes[4] > 1 or comptes[5] > 1 or comptes[6] > 1:
            return True
        else:
            return False
